...
/**********************************************************
***	Emm_V5.0步进闭环控制例程 - 修改版
***	修改：统一使用USART_SendArray接口，支持自选串口
***	编写作者：ZHANGDATOU
***	技术支持：张大头闭环伺服
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void Emm_V5_Origin_Modify_Params(USART_TypeDef* USARTx, uint8_t addr, bool svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, bool potF)
{
  uint8_t cmd[32] = {0};
